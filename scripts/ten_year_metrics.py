import csv, datetime as dt, io, json, math, pathlib, urllib.request, zipfile
...
HEAD={'User-Agent':'Mozilla/5.0 radar-b3-ia/4.0','Accept-Language':'pt-BR,pt;q=0.9'}
...
# Fechamento do último pregão de cada ano, fonte COTAHIST/B3
...
# Proventos por data ex, fonte pública StatusInvest
def dividends(ticker):
 ...
 return annual
# Receita consolidada 10 anos, fonte CVM DFP
def historical_revenue(year):
 raw=get(f'https://dados.cvm.gov.br/dados/CIA_ABERTA/DOC/DFP/DADOS/dfp_cia_aberta_{year}.zip')
 out={}
 with zipfile.ZipFile(io.BytesIO(raw)) as z:
  name=next(n for n in z.namelist() if 'DRE_con' in n and n.endswith('.csv'))
  reader=csv.DictReader(io.TextIOWrapper(z.open(name),encoding='latin-1'),delimiter=';')
  for r in reader:
...
for x in items:
 ...
 x['dy_10y_avg']=sum(yields)/10;x['dy_10y_history']=history;x['revenue_growth_10y']=((current/base)**(1/10)-1)*100;x['revenue_10y_base']=base;x['revenue_10y_current']=current
 x['dy_10y_source']='StatusInvest proventos por data ex + fechamento anual COTAHIST/B3';x['growth_10y_source']=f'CVM DFP consolidada {start_year} e {end_year}';x['ten_year_period']=f'{start_year}-{end_year}';kept.append(x)
if len(kept)<10:raise RuntimeError(f'Apenas {len(kept)} ações têm série completa de 10 anos')
D['items']=kept;D['universe_size']=len(kept);D['ten_year_period']=f'{start_year}-{end_year}';D['ten_year_coverage']=len(kept);D['message']=D.get('message','')+f' Séries de 10 anos completas em {len(kept)} ações.'
P.write_text(json.dumps(D,ensure_ascii=False,allow_nan=False),encoding='utf-8');print('10 ANOS PASS',len(kept),start_year,end_year)
